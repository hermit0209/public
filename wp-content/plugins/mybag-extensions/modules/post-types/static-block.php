		...
		static function init_static_blocks() {
			if (function_exists('register_post_type')) {
				register_post_type('static_block',
					array(
						'labels' => array(
							'name' 					=>	esc_html_x('Static Content Blocks', 'post type general name', 'mybag-extensions'),
							'singular_name' 		=>	esc_html_x('Static Block', 'post type singular name', 'mybag-extensions'),
							'add_new' 				=>	esc_html_x('Add New', 'block', 'mybag-extensions'),
							'add_new_item' 			=>	esc_html__('Add New Block', 'mybag-extensions'),
							'edit_item' 			=>	esc_html__('Edit Block', 'mybag-extensions'),
							'new_item' 				=>	esc_html__('New Block', 'mybag-extensions'),
							'all_items' 			=>	esc_html__('Static Blocks', 'mybag-extensions'),
							'view_item' 			=>	esc_html__('View Block', 'mybag-extensions'),
							'search_items' 			=>	esc_html__('Search', 'mybag-extensions'),
							'not_found' 			=>	esc_html__('No blocks found', 'mybag-extensions'),
							'not_found_in_trash' 	=>	esc_html__('No blocks found in Trash', 'mybag-extensions'), 
							'parent_item_colon' 	=> '',
...
$meta_box_static_blocks = array(
	...
	'title' 	=>  esc_html__('Content Options', 'mybag-extensions'),
	'page' 		=> 'static_block',
	'context' 	=> 'side',
	'priority' 	=> 'default',
	'fields' 	=> array(
    	array(
    	   'name' 		=> esc_html__('Content Filters', 'mybag-extensions'),
    	   'desc' 		=> esc_html__('Apply all WP content filters? This will include plugin added filters.', 'mybag-extensions'),
    	   'id' 		=> 'content_filters',
    	   'type' 		=> 'radio',
    	   'std' 		=> '',
    	   'options'	=> array(
    	   		'default'	=> esc_html__('Defaults (recommended)', 'mybag-extensions'),
    	   		'all' 		=> esc_html__('All Content Filters', 'mybag-extensions')
    	   	)
    	),
    	array(
    	   'name' 		=> esc_html__('Auto Paragraphs', 'mybag-extensions'),
    	   'desc' 		=> esc_html__('Add &lt;p&gt; and &lt;br&gt; tags automatically. (disabling may fix layout issues)', 'mybag-extensions'),
    	   'id' 		=> 'wpautop',
    	   'type' 		=> 'radio',
    	   'std' 		=> '',
    	   'options' 	=> array(
    	   		'on' 	=> esc_html__('On', 'mybag-extensions'),
    	   		'off' 	=> esc_html__('Off', 'mybag-extensions')
    	   	)
    	)
   	)
);
...
function mybag_add_box_static_blocks() {
	global $meta_box_static_blocks;
	
	add_meta_box(
		$meta_box_static_blocks['id'], 
		$meta_box_static_blocks['title'], 
		'mybag_show_box_static_blocks', 
...
add_action('admin_menu', 'mybag_add_box_static_blocks');
...
function mybag_show_box_static_blocks() {
	global $meta_box_static_blocks, $post;
 	
	// Use nonce for verification
	echo '<input type="hidden" name="mybag_meta_box_nonce" value="', wp_create_nonce( plugin_basename(__FILE__) ), '" />';
...
add_action('save_post', 'mybag_save_data_static_blocks');
...
function mybag_save_data_static_blocks($post_id) {
	global $meta_box_static_blocks;
 
	// verify nonce
	if ( !isset($_POST['mybag_meta_box_nonce']) || !wp_verify_nonce($_POST['mybag_meta_box_nonce'], plugin_basename(__FILE__))) {
		return $post_id;
	}
